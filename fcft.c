...
font_size_adjust(const struct font *_font, double amount)
{
    ...
    if (FcPatternGetDouble(font->fc_pattern, FC_SIZE, 0, &size) != FcResultMatch)
        LOG_WARN("%s: failed to get size", font->name);
    ...
    /* Append ":size=" */
    char new_size[32];
    snprintf(new_size, sizeof(new_size), ":size=%.0f", size);
    pattern = realloc(pattern, strlen(pattern) + strlen(new_size) + 1);
    strcat(pattern, new_size);

    /* Hash the new pattern. This will typically *not* match hashes
     * generated by font_from_name() */
    uint64_t hash = 0;
    hash ^= sdbm_hash(pattern);
    tll_foreach(font->fallbacks, it)
        hash ^= sdbm_hash(it->item.pattern);

    /* Check if we've already instantiated this pattern */
    tll_foreach(font_cache, it) {
        if (it->item.hash == hash) {
            it->item.font->ref_counter++;
            free(pattern);
    ...
    /* Copy fallback patterns (NOT instantiated fallbacks fonts) */
    /* TODO: adjust size of fallback fonts */
    tll_foreach(font->fallbacks, it) {
        tll_push_back(
            new_font->fallbacks,
            ((struct font_fallback){.pattern = strdup(it->item.pattern)}));
    }

    /*
     * Make this one *stick* in the cache. This means that resizing
     * the font "back-and-forth" will find it in the cache
     */
    new_font->ref_counter++;

    /* Add new font instance to cache */
    tll_push_back(font_cache, ((struct font_cache_entry){.hash = hash, .font = new_font}));
